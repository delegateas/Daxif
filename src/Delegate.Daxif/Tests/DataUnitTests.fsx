...
let tc6 () =
    try
      match (CrmDataInternal.Entities.retrieveSolution p @"XrmOrg").Id with
      | guid -> true
    with
      | _ -> false
...
let tc7 () =
    try
      match (CrmDataInternal.Entities.retrieveSolution p @"XrmOrg").Id with
      | guid -> 
        CrmDataInternal.Entities.retrieveWebResources p guid
        |> Seq.length > 0
    with
      | _ -> false
...
let tc8 () =
    (CrmDataInternal.Entities.retrieveRootBusinessUnit p)
...
let tc9 () =
    (CrmDataInternal.Entities.retrieveBusinessUnit p AuthInfo.organization)
        .Attributes.[@"name"] :?> string = AuthInfo.organization
