...
// Function for creating SDK proxy from an SDK Client
...
(* Create query to fetch the entities from CRM *)
let createRetrievEntitieReq logicName =
...
(* Fetch Entities from dev *)
let entities' = 
  entities
  |> Array.map createRetrievEntitieReq
...
(* Update entity references in entities *)
let entities''=
  entities'
  |> Seq.map(fun e -> 
    let a'' = 
